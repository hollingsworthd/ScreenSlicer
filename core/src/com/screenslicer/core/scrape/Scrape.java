...
public class Scrape {
  ...
  private static void initFirefox(Request req, Proxy[] proxies) throws UnsupportedEncodingException {
  ...
  private static void start(Request req) {
    ...
    for (int i = 0; i < RETRIES; i++) {
      try {
        if (Request.BROWSER_FIREFOX.equals(req.browser)) {
          initFirefox(req, proxies);
        }
        try {
